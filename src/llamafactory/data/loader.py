...
from .data_utils import preprocess_sp_dataset, preprocess_sp_multimodal
...
def sequence_parallel_decorator(get_dataset):
    ...
    def sequence_parallel_processor(*args, **kwargs):
        ...
        if model_args.sequence_parallel_size > 1:
            ...
            def pad_sequence(examples):
                ...
                for k, v in examples.items():
                    ...
                    examples[k] = [
                        seq + [pad_token_id] * (max_length - len(seq)) for seq in v
                    ]
            ...
            def sp_split(examples):
                ...
                if "images" in keys or "videos" in keys:
                    # convert key: list to list of dicts
                    converted_examples = [
                        dict(zip(keys, row)) for row in zip(*examples.values())
                    ]
                    for key in keys:
                        examples[key] = []

                    for example in converted_examples:
                        splitted_row = preprocess_sp_multimodal(
                            example,
                            model_args.sequence_parallel_size,
                            model_args.sequence_parallel_mode,
                            mm_plugin,
                            processor,
                        )
                        for k in keys:
                            examples[k] += splitted_row[k]
        ...
        # these are all the correct sequence length
        # for k in list(dataset_module["train_dataset"].features.keys()):
        #     if k in ["input_ids", "position_ids", "attention_mask", "labels"]:
        #         for i in range(len(dataset_module["train_dataset"][k])):
        #             if len(dataset_module["train_dataset"][k][i]) != 8000:
        #                 if k == "position_ids" and len(dataset_module["train_dataset"][k][i][0][0]) == 8000:
        #                     continue
        #                 print(f"In Loader: {k}, {i}, {len(dataset_module['train_dataset'][k][i])}")
...
def get_dataset(
    ...
    with training_args.main_process_first(desc="load dataset"):
        ...
        eval_dataset = _get_merged_dataset(
            ...
            training_args,
            stage,
            merge=training_args.do_predict,
        )
