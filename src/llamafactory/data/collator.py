...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, "torch.Tensor"]:
        ...
        if (
            ...
            _fake_input_ids = self.tokenizer.encode(
        ...
        if (
            self.template.mm_plugin.audio_token is not None and sum(batch_audlens) == 1
        ):  # avoid process hanging in zero3/fsdp case
        ...
        if len(fake_input_ids) != 0:
            input_ids_np = np.array(features[0]["input_ids"])
            pad_locations = input_ids_np == self.tokenizer.pad_token_id
            if sum(pad_locations) <= len(fake_input_ids):
                print(self.tokenizer.decode(features[0]["input_ids"]))

            assert sum(pad_locations) > len(
                fake_input_ids
            ), f"must have enough pad to insert fake input_ids got: {sum(pad_locations)}, {len(fake_input_ids)}. Total len: {input_ids_np.shape[0]}"

            # assume after first pad it's all pads
            end_idx = np.max(np.where(pad_locations)[0])
            start_idx = end_idx - len(fake_input_ids)
            # start_idx = np.min(np.where(pad_locations)[0])
            # end_idx = start_idx + len(fake_input_ids)

            if self.tokenizer.padding_side == "right":
                features[0]["input_ids"] = (
                    features[0]["input_ids"][:start_idx]
                    + fake_input_ids
                    + features[0]["input_ids"][end_idx:]
                )
                features[0]["attention_mask"] = (
                    features[0]["attention_mask"][:start_idx]
                    + ([0] * len(fake_input_ids))
                    + features[0]["attention_mask"][end_idx:]
                )
                features[0]["labels"] = (
                    features[0]["labels"][:start_idx]
                    + ([IGNORE_INDEX] * len(fake_input_ids))
                    + features[0]["labels"][end_idx:]
                )
            else:
                features[0]["input_ids"] = fake_input_ids + features[0]["input_ids"]
                features[0]["attention_mask"] = [0] * len(fake_input_ids) + features[0][
                    "attention_mask"
                ]
                features[0]["labels"] = [IGNORE_INDEX] * len(fake_input_ids) + features[
                    0
                ]["labels"]
        ...
        if (
            ...
            and hasattr(self.model, "get_rope_index")
            and self.require_position_ids
            and len(features["position_ids"].shape) == 4
        ):
            features["position_ids"] = features["position_ids"].transpose(0, 2)[0]
        # change
        if (
            self.model is not None
            and hasattr(self.model, "get_rope_index")
            and not self.require_position_ids
        ):  # for qwen2vl mrope
            ...
            if "second_per_grid_ts" in mm_inputs:
                rope_index_kwargs["second_per_grid_ts"] = mm_inputs.get(
                    "second_per_grid_ts"
                )

            features["position_ids"], features["rope_deltas"] = (
                self.model.get_rope_index(**rope_index_kwargs)
            )
        ...
        #     print(f"In Collator: {k}, {features[k].shape}")
...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: Sequence[Dict[str, Any]]) -> Dict[str, "torch.Tensor"]:
        if not self.require_position_ids:
            features = [
                {k: v for k, v in d.items() if k != "position_ids"} for d in features
            ]
